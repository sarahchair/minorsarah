...
class model_broadcast extends CI_Model {
	...
	{
		$query = $this->db->get_where('broadcast', array('id_broadcast' => $id));
		return $query->row_array();
	}
	/*public function get_data_email($email){
		$query = $this->db->get_where('subscriber', array('email_subscriber' => $email));
		return $query->row_array();
	} */
	...
	public function gettablesubscriber($tablename){
		//return $this->db->query("SELECT * FROM $tablename");->result_array();
		$datasubscriber = $this->db->query("SELECT * FROM $tablename");
		return $datasubscriber->result_array();
	}

	public function delete_data($id)
	{
		return $this->db->delete('broadcast', array('id_broadcast'=>$id));
	}
}
